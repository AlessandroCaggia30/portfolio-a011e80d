"""Build AI walkthrough plot for Ex 4.1a — P(X > 10) for X ~ N(8, 1.2^2).

Visual: Normal density centered at mu=8 with sigma=1.2, with the right-tail
P(X > 10) shaded. Side panel shows the same probability in standard-normal
coordinates (z = (10-8)/1.2 = 1.6667), connecting the original scale to the
z-score. Reinforces both the geometric meaning of a tail probability and the
location of the threshold relative to mu and sigma.
"""
import os, sys
...
OUT = "/Users/Alessandro/Repos/portfolio-a011e80d/apps/mindnotes/statistics/images/ex4/ex4_1a_ai.png"

mu, sigma = 8.0, 1.2
thr       = 10.0
z_thr     = (thr - mu) / sigma                  # 1.6667
p_tail    = 1.0 - norm.cdf(thr, mu, sigma)      # 0.04779
...
# shade right tail x > 10
xs_tail = np.linspace(thr, mu + 4*sigma, 200)
ax1.fill_between(xs_tail, 0, norm.pdf(xs_tail, mu, sigma),
                 color=PALETTE["warn"], alpha=0.35,
                 label=fr"$\Pr(X>10) = {p_tail:.4f}$")
...
ax1.text(thr, y_top*0.62, "  glass limit\n  $x=10$ cl",
         color=PALETTE["warn"], ha="left", va="center", fontsize=11,
         fontweight="bold")
...
ax1.set_title(r"$X \sim N(8,\, 1.2^2)$ — right tail beyond glass limit")
...
zs_tail = np.linspace(z_thr, 4, 200)
ax2.fill_between(zs_tail, 0, norm.pdf(zs_tail),
                 color=PALETTE["warn"], alpha=0.35,
                 label=fr"$\Pr(Z>{z_thr:.4f}) = {p_tail:.4f}$")
...
ax2.set_title(r"Same tail in $Z$-coordinates  —  threshold is $1.67\,\sigma$ above $\mu$")

...
\emph{Closed form via DLMwR \S 1.5 (Bayesian linear regression).} The prior $\theta_t\mid y_{1:t-1}\sim\mathcal{N}_p(a_t,R_t)$ and the Gaussian likelihood $Y_t\mid\theta_t\sim\mathcal{N}_q(F_t\theta_t,V_t)$ form a conjugate Normal--Normal pair. The book derives the closed-form posterior in \S 1.5 (Bayesian linear regression / Normal--Normal conjugate update) and applies the result here. The posterior mean and variance are:
\[ \boxed{\;\theta_t\mid y_{1:t}\sim\mathcal{N}_p(m_t,C_t),\quad m_t=a_t+R_t F_t^{\top}Q_t^{-1}(y_t-f_t),\;C_t=R_t-R_t F_t^{\top}Q_t^{-1}F_t R_t.\;} \]
Defining the \textbf{Kalman gain} $K_t=R_t F_t^{\top}Q_t^{-1}$, this rewrites as $m_t=a_t+K_t(y_t-f_t)$ and $C_t=R_t-K_t Q_t K_t^{\top}$.
